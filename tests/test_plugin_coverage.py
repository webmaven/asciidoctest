...
def test_plugin_unparseable_asciidoc_collection(pytester):
    ...
    result.stdout.fnmatch_lines([
        "*Failed to parse AsciiDoc file*Mock parser failure*"
    ])
...
def test_plugin_invalid_python_syntax_and_import_errors(pytester):
    ...
    pytester.makepyfile(invalid_syntax="""\
    ...
    pytester.makepyfile(module_with_import_error="""\
        '''
        [source,python,test]
        ----
        >>> True
    ...
    pytester.makepyfile(unparseable_docstring="""\
        '''
        [source,python
        ----
        mismatched brackets
...
def test_plugin_repr_non_custom_failure(pytester):
    ...
    pytester.makepyfile(generic_failure="""\
        '''
        [source,python,test]
        ----
